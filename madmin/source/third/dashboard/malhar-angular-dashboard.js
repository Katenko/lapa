  ...
  .directive('dashboard', ['WidgetModel', 'WidgetDefCollection', '$modal', 'DashboardState', '$log', function (WidgetModel, WidgetDefCollection, $modal, DashboardState, $log) {
    ...
    return {
      ...
      templateUrl: function(element, attr) {
        return attr.templateUrl ? attr.templateUrl : 'third/dashboard/dashboard.html';
      },
      scope: true,
      ...
      controller: ['$scope', '$attrs', function (scope, attrs) {
        ...
        var defaults = {
          ...
          settingsModalOptions: {
            templateUrl: 'third/dashboard/widget-settings-template.html',
  ...
  .controller('DashboardWidgetCtrl', ['$scope', '$element', '$compile', '$window', '$timeout',
    function($scope, $element, $compile, $window, $timeout) {
      ...
      $scope.maxResizer = function (e) {
        ...
        if ($scope.maximizedThis) {
          widget.setWidth($scope.widget.previousWidth);
          widget.setHeight($scope.widget.previousHeight);
          $scope.widget.previousWidth = widget.size.width;
          $scope.widget.previousHeight = widget.size.height;

          $scope.$parent.maximizedOther = false;
          $scope.maximizedThis = false;
          ...
          var ht_diff = 180;  // height differential, or zero if you want to fill out the entire brower window
          $scope.widget.previousWidth = widget.size.width;
          $scope.widget.previousHeight = widget.size.height;

          widget.setWidth(window.innerWidth);
          $scope.widget.setHeight(window.innerHeight - ht_diff);
          $scope.$parent.maximizedOther = true;
          $scope.maximizedThis = true;
  ...
  .directive('dashboardLayouts', ['LayoutStorage', '$timeout', '$modal',
    function(LayoutStorage, $timeout, $modal) {
      return {
        scope: true,
        templateUrl: function(element, attr) {
          return attr.templateUrl ? attr.templateUrl : 'third/dashboard/dashboardLayouts.html';
        },
          ...
          scope.makeLayoutActive = function(layout) {
            ...
            if (current && current.dashboard.unsavedChangeCount) {
              var modalInstance = $modal.open({
                templateUrl: 'third/dashboard/SaveChangesModal.html',
                resolve: {
                  layout: function() {
                    return layout;
                  }
